...
mkState name = FilterState
  -- { _editor = editorText (UIName "editor-fzf") (str . (concatMap unpack)) (Just 1) ""
  { _editor = editorText name (str . (concatMap unpack)) (Just 1) ""
  , _isFocused = False
  , _isActive = False
  }

-- mkWidget :: Editor -> Widget UIName
-- mkWidget :: FilterState -> Widget UIName
...
-- handleEvent :: BrickEvent n e -> FilterState n -> FilterState n
-- handleEvent :: Vty.Event -> FilterState n -> EventM n (Next (FilterState n))
handleEvent :: Vty.Event -> FilterState n -> EventM n (FilterState n)
-- handleEvent event state = state & editorL %~ (handleEditorEvent event)
-- handleEvent event state = continue =<< (\m -> state & editorL .~ m) <$> handleEditorEvent event (state^.editorL)
