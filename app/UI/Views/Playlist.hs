...
import UI.Types (AppState, ViewType(..), playlist, activeView, VtyEvent(..), UIName(..))
...
event state e = case e of
    VtyEvent (V.EvKey (V.KChar 'j') []) -> next state
    VtyEvent (V.EvKey (V.KChar 'k') []) -> previous state
    VtyEvent (V.EvKey V.KEnter []) -> play state
    VtyEvent (V.EvKey (V.KChar 'q') []) -> halt state
    -- VtyEvent (V.EvKey (V.KChar '1') []) -> changeView state 1
    {-V.EvKey (V.KChar '-') [] -> delete-}
    VtyEvent ev -> listEvent ev state
